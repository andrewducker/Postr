...
public class CookieHandler {
	...
	static {
		SiteData siteData;
		try {
			siteData = DAO.GetSiteData();
			key = siteData.HMACKey;
			cookieTimeout = siteData.CookieTimeout;
		} catch (Exception e) {
			LogHandler.logException(CookieHandler.class, e, "Failed to get site data");
		}
	}
	
	private static String key;
	
	private static long cookieTimeout;
	...
	public static String ParseCookie(String contents) throws Exception{
		...
		String hmac = contents.substring(parts[0].length() + parts[1].length() + 2);
		
		//Then we compare the HMAC, to make sure it's valid.
		String testedHmac = computeHmac(email+"|"+expires+"|"); 
		if (!hmac.equals(testedHmac)) {
			LogHandler.logWarning(CookieHandler.class, "Failed to parse HMAC for " + contents);
			return null;
		}
		DateTime cookieExpires = new DateTime(expires);
		if (!cookieExpires.isAfterNow()) {
			LogHandler.logWarning(CookieHandler.class, "Cookie expired at " + cookieExpires.toString());
			return null;
		}
